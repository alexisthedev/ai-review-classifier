...
class DecisionTree:
    ...
    def calculate_gini(self, left_labels, right_labels):
        ...
        gini_left = 1.0 - sum((np.sum(left_labels == label) / len(left_labels))**2 for label in np.unique(left_labels))
        gini_right = 1.0 - sum((np.sum(right_labels == label) / len(right_labels))**2 for label in np.unique(right_labels))
