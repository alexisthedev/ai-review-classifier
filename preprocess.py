...
class Preprocess:
    ...
    def preprocess_reviews(self):
        ...
        word_index = tf.keras.datasets.imdb.get_word_index() # dict {index : word}
        index2word = dict((i + 3, word) for (word, i) in word_index.items())
        index2word[0] = '[pad]'
        index2word[1] = '[bos]'
        index2word[2] = '[oov]'
        x_train_imdb = np.array([' '.join([index2word[idx] for idx in text]) for text in x_train_imdb]) # get string from indeces
        x_test_imdb = np.array([' '.join([index2word[idx] for idx in text]) for text in x_test_imdb]) # get string from indeces
